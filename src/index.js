        ...
        reportTaskDone(endTime, passed, warnings) {
            ...
            if (this.failedTests.length) {
                this.newline()
                    ...
                    .write(
                        this.chalk.bold.red(
                            `Errors summary (${this.failedTests.length}):`
                        )
                    )
                    .newline();

                const symbol = this.chalk.red.bold(this.symbols.err);
                const nameStyle = this.chalk.red.bold;

                this.failedTests.forEach((ft) => {
                    const title = `${symbol} ${nameStyle(ft.testName)}`;
